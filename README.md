...
| Blender 脚本 | `scripts/`（build_housing / build_pack 等） |
...
| 刀模/素材接入（PNG/JPG/SVG/PDF） | `dieline.py`（PDF 矢量 + 扫描件栅格化回退） | SVG 44/44、PDF 55/55 折线召回 100% |
| OCR 文字/主视觉识别 | `scripts/mvp/vision.py`（Windows OCR + 连通域主视觉 + 透明区） | 中文 OCR 实测正确；透明图检出 84.3%；自动生成品牌锁定区域 |
...
| `scripts/make_fefco_dieline.py` | 素材预处理（工业刀版图） |
## 技术路线要点

- 几何走**参数化模板/脚本**（回转体、盒体），LLM 一句话 + 迭代修改即可重建；
- 贴图用**分面映射**（包装盒）与 6 类材质预设；
